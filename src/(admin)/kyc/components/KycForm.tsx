...
import {
	...
	type AddKycDTO,
	emptyKyc,
} from "../schemas/kycSchema";
import { useQuery, useQueryClient } from "@tanstack/react-query";

export type KycFormProps = {
	editData?: AddEditKycDTO;
};
...
const fetchKycRequiredFields = async () => {
	const data: PostCallData = { endpoint: "kyc_required_fields" };
	const response = await apiPostCall(data);
	if (response?.status === API_CONSTANTS.success) {
		return response.data;
	}
	throw new Error("Failed to fetch KYC required fields");
};

export default function KycForm({ editData }: KycFormProps) {
	...
	useEffect(() => {
		if (editData?.kycNumber) form.reset(editData);
	}, [form, editData]);
	...
	const { data: kycRequiredFields } = useQuery({
		queryKey: ["kycRequiredList"],
		queryFn: fetchKycRequiredFields,
		staleTime: 5 * 60 * 1000,
		gcTime: 10 * 60 * 1000,
		retry: 2,
	});
	...
	const tabs = [
		...
		},
		{
			id: 6,
			name: "Documents",
			component: <DocumentDetails form={form} data={editData} />,
		},
	];
	...
	const handleContinue = async () => {
		...
		if (currentStep < tabs.length - 1) {
			...
			} else if (isFormValid && editData?.kycNumber) {
