...
type Props = {
	receiptNo?: SelectOption[];
};

export const RepaymentReceiptForm = ({ receiptNo }: Props) => {
	const [isSubmitting, setIsSubmitting] = useState(false);
	const router = useRouter();
	const [receiptList, setReceiptList] = useState<{ receiptNo?: string }>({});
	const form = useForm<RepaymentReceiptFormDTO>({
		defaultValues: emptyRepaymentReceipt,
		resolver: zodResolver(RepaymentReceiptSchema),
	});

	const getReceiptDetail = useCallback(async (receiptNo: string) => {
		try {
	...
	const handleSubmit = () => {
		router.push(`repayment-receipt/view/1213`);
	};
