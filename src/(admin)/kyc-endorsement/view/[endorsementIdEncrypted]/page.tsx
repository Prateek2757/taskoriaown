...
export default function Page() {
	...
	const endorsementId = params.endorsementIdEncrypted;
	console.log("sudeep", params);
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				setLoading(true);
				const data = {
					endorsementIdEncrypted: endorsementId || null,
					endpoint: "endorsement_view",
				};

				console.log("View Data: ", endorsementId);
		...
		<div className="min-h-screen">
			<div className="">
				...
				{loading ? (
					...
					<div className="bg-white rounded-lg border-1 p-6 text-center">
						<p>No KYC details found for number: {endorsementId}</p>
					</div>
				)}
			</div>
		</div>
