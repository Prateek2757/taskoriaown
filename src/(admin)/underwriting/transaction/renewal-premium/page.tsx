...
import { emptyRenualPremium, RenualPremiumDTO, RenualPremiumSchema } from "./RenualPremiumSchema";
import Usevalidation from "@/hooks/Admin/use-validation";

export default function InsurancePremiumUI() {
  const [renewalProposalList, setRenewalProposalList] = useState<SelectOption[]>([]);
  const [peerbankList, setpeerbankList] = useState<SelectOption[]>([]);
  const [bankName, setBankName] = useState<SelectOption[]>([]);
  const [partialPayments, setPartialPayments] = useState<RenualPremiumDTO>([]);
  const { showToast } = useToast();
  const route = useRouter();

  
  const form = useForm({
    resolver: zodResolver(RenualPremiumSchema),
    defaultValues: emptyRenualPremium(),
    mode: "all",
  });
  ...
  const onSubmit: SubmitHandler<RenualPremiumDTO> = async (
    formData: RenualPremiumDTO
  ) => {
