...
export function ReferralsPage() {
  ...
  } = useSWR<ReferralsResponse>(
    currentUser ? "/api/referrals/otherDetails" : null,
    fetcher
  );
  ...
  const handleShare = (platform: string) => {
    const text = encodeURIComponent(
      `Join me! Use my referral link to sign up:`
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <Card key={stat.label} className="border-0 py-0 shadow-sm">
            <CardContent className="p-3 flex items-center gap-3">
              <div
                className={`${stat.bg} w-10 h-10 rounded-xl flex items-center justify-center shrink-0`}
              >
                {stat.icon}
              </div>
              <div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell>
                              <Badge
                                variant="secondary"
                                className={`${cfg.color} border text-xs font-semibold gap-1 py-0.5`}
                              >
                                {r.status === "pending" ? (
