"use client";
import { CheckCircle, Info, HelpCircle, DollarSign, Lightbulb, Sparkles, Clock, Star, TrendingUp } from 'lucide-react';
    ...
    headers.forEach((h2) => {
      ...
      if (text.includes('get quotes') || text.includes('how')) {
        ...
        if (nextElement && nextElement.tagName === 'UL') {
          const items = nextElement.querySelectorAll('li');
          sections.whatsIncluded = Array.from(items).map(li => li.textContent);
        }
        ...
        while (current && current.tagName !== 'h2') {
          if (current.tagName === 'p') {
            ...
            sections.pricing.factors = Array.from(items).map(li => li.textContent);
          }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-14">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            {sections.howItWorks.map((step, i) => (
              ...
              >
                <div className={`absolute -top-8 left-1/2 -translate-x-1/2 w-16 h-16 bg-gradient-to-br ${gradients[i]?.num} rounded-2xl flex items-center justify-center text-white font-black text-3xl shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}>
                  {i + 1}
                </div>
                <div className="pt-10">
                ...
                {/* Decorative element */}
                <div className={`absolute -bottom-2 -right-2 w-24 h-24 bg-gradient-to-br ${gradients[i]?.num} rounded-full opacity-10 blur-2xl group-hover:opacity-20 transition-opacity duration-500`} />
              </div>
            ))}
          </div>
        </div>
      ...
      {sections.whatsIncluded.length > 0 && (
        <div className="mb-14">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#3C7DED] via-[#41A6EE] to-[#46CBEE] border border-emerald-200/50 dark:border-emerald-700/50 text-white font-bold rounded-full text-sm mb-6 backdrop-blur-sm">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {sections.whatsIncluded.map((item, i) => (
              ...
              >
                ...
                {/* Bottom accent line */}
                <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#3C7DED] via-[#41A6EE] to-[#46CBEE] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
              </div>
            ))}
          </div>
        </div>
      ...
      {sections.pricing.factors.length >= 0 && (
        <div className="mb-14">
          <div className="text-center ">
            ...
            <h2 className="text-4xl md:text-5xl font-extrabold pb-2 text-transparent bg-clip-text bg-gradient-to-r from-[#3C7DED] via-[#41A6EE] to-[#46CBEE]  mb-6">
              Fair & Clear Pricing
            </h2>
            ...
            <div className="flex items-center justify-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span className="font-semibold">Compare quotes from multiple professionals</span>
              <Star className="w-5 h-5 text-white fill-blue-500" />
            </div>
          </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 via-blue-50/50 to-[#46CBEE]/30 dark:from-amber-950/30 dark:via-orange-950/20 dark:to-yellow-950/30 rounded-3xl p-8 border-2 border-amber-200/50 dark:border-amber-800/50 backdrop-blur-sm">
              <div className="flex flex-col md:flex-row items-center justify-around gap-6 text-center">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="font-bold text-gray-900 dark:text-white text-lg">No Hidden Fees</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">What you see is what you pay</p>
                </div>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="font-bold text-gray-900 dark:text-white text-lg">Vetted Professionals</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">All providers are verified</p>
                </div>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="font-bold text-gray-900 dark:text-white text-lg">Best Value</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Competitive market rates</p>
                </div>
              </div>
            </div>
          </div>
