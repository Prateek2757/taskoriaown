...
export default function TasksList({ tasks }: { tasks?: Task[] }) {
  ...
  const [responseCache, setResponseCache] = useState<Record<number, Response[]>>({});
  ...
  const updateStatus = useCallback(
    async (taskId: number, status: string, closeReason?: string, questions?: any) => {
      try {
        setSaving(true);
        ...
        const data = res.data;
        if (data?.success) {
          toast.success(status === "Closed" ? "Task closed successfully" : "Status updated");
        } else {
  ...
  const loadResponses = useCallback(async (taskId: number) => {
  ...
  const handleCloseConfirm = useCallback(
    async (questions: Record<string, string>, comments: string) => {
      ...
      setLocalStatuses((prev) => ({ ...prev, [pendingCloseTask.task_id]: "Closed" }));
      await updateStatus(pendingCloseTask.task_id, "Closed", comments, questions);
  ...
  const isSearchingOrFiltering = searchQuery.trim() !== "" || statusFilter !== "all";
  
  const displayTasks = isSearchingOrFiltering 
    ? filteredTasks 
    : filteredTasks?.slice(0, visibleCount);
  
  const hasMoreTasks = !isSearchingOrFiltering && filteredTasks && filteredTasks.length > visibleCount;
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center space-y-4 max-w-md">
          ...
          <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => router.push("/create-task")}>
        ...
        <div className="flex gap-6 text-sm">
          ...
          <div>
            ...
            <span className="font-semibold text-slate-900">
              ...
              {!isSearchingOrFiltering && filteredTasks && filteredTasks.length > visibleCount && 
