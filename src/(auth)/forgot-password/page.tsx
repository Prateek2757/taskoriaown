...
import { type ForgorPassordPayload, forgotPassword } from "@/helper/apiService";
import { type GenerateOtpDTO, GenerateOtpFormSchema } from "../authSchema";
...
export default function Page() {
	...
	const onSubmit: SubmitHandler<GenerateOtpDTO> = async (data) => {
		...
		try {
			const payload: ForgorPassordPayload = {
				...data,
			};
			...
			if (response && response.status === API_CONSTANTS.success) {
				if (response.data.code === SYSTEM_CONSTANTS.success_code) {
					...
					router.push(`/en/verify-otp-forgotpassword`);
				} else {
