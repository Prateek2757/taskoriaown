...
import {
	type VerifyOtpForgotPasswordPayload,
	verifyOtpForgotPassword,
} from "@/helper/apiService";
import { useLanguage } from "@/hooks/use-changeLanguage";
import { type VerifyOtpFormDTO, VerifyOtpFormSchema } from "../authSchema";
...
export default function Page() {
	...
	const onSubmit: SubmitHandler<VerifyOtpFormDTO> = async (data) => {
		...
		try {
			const payload: VerifyOtpForgotPasswordPayload = {
				mobileNumber: window.sessionStorage.getItem("mobile-phone") || "",
				otp: data.otp,
			};

			const response = await verifyOtpForgotPassword(payload);
			...
			if (response?.data.code === SYSTEM_CONSTANTS.success_code) {
				...
				router.push("/en/change-password");
			} else {
					...
					<form onSubmit={form.handleSubmit(onSubmit)}>
						<div className="space-y-6">
							<div>
								...
								>
								 {locale==='ne'? "जारी राख्न OTP प्रविष्ट गर्नुहोस्":"Enter OTP to Continue"}	
								</label>
								<Controller
									name="otp"
